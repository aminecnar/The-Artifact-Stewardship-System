...
ArtifactNode *AVLTree::rotateLeft(ArtifactNode *x)
{
    ...
    int height_x_left=height(x->left);
    int height_x_right=height(x->right);
    x->height=1+(height_x_left > height_x_right ? height_x_left:height_x_right);

    int height_y_left=height(y->left);
    int height_y_right=height(y->right);
    y->height=1+(height_y_left > height_y_right ? height_y_left:height_y_right);
    return y;
}
...
ArtifactNode *AVLTree::remove(ArtifactNode *node, int artifactID, bool &removed)
{
    ...
    if(artifactID < node->data.artifactID){
        ...
        if((node->left==nullptr) || (node->right==nullptr)){
            ...
            node->right = remove(node->right, temp->data.artifactID, subRemoved);
        }
    }
    if(node==nullptr){
        return node;
